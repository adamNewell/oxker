...
use oxker_core::{EventBus, CoreEvent, events::types::{Stats, LogLine, ContainerItem}};
...
async fn test_event_handler_receives_container_stats_update() {
    ...
    event_bus.publish(CoreEvent::ContainerStatsUpdate {
        container_id: "test-container-1".to_string(),
        stats,
    }).await;
...
async fn test_event_handler_receives_logs_update() {
    ...
    event_bus.publish(CoreEvent::ContainerLogsUpdate {
        container_id: "test-container-1".to_string(),
        logs,
    }).await;
...
async fn test_event_handler_receives_error_event() {
    ...
    // Publish error event
    event_bus.publish(CoreEvent::Error("Test error message".to_string())).await;
...
async fn test_multiple_events_processed_in_order() {
    ...
    event_bus.publish(CoreEvent::ContainerRemoved("test-container-2".to_string())).await;
