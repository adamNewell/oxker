...
fn main() {
    ...
    for i in 1..=10 {
        ...
        container_state.lock().update_container_stats(format!("container_{}", i), stats);
    }
    
    // Benchmark view model creation
    let iterations = 10000;
    println!("Running {} iterations of FrameViewModel creation...", iterations);
    
    let start = Instant::now();
    for _ in 0..iterations {
        let gui_data = gui_state.lock();
        let container_data = container_state.lock();
        let _frame_view_model = FrameViewModel::from_state(
            &container_data,
            &gui_data,
            config.app_colors,
            160
    ...
    println!("Approximate size: ~{} bytes", std::mem::size_of_val(&frame_view_model));
}