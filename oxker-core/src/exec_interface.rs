...
impl ExecSession {
    ...
    pub fn new(container_id: Arc<ContainerId>, docker: Arc<Docker>, use_cli: bool) -> Self {
        Self {
            container_id,
            docker,
            use_cli,
...
pub mod security {
    ...
    pub fn sanitize_tty_input(input: &[u8]) -> Result<Vec<u8>, AppError> {
        ...
        while i < input.len() {
            match input[i] {
                ...
                0x20..=0x7E => sanitized.push(input[i]),
                // Allow common control characters
                0x09 | 0x0A | 0x0D => sanitized.push(input[i]), // Tab, LF, CR
    ...
    fn is_safe_csi_sequence(input: &[u8]) -> bool {
        ...
        let mut i = 2;
        while i < input.len() && i < 20 { // Limit sequence length
            match input[i] {
                // Safe movement commands
                b'A' | b'B' | b'C' | b'D' | b'H' | b'F' => return true,
                // Safe editing commands
                b'K' | b'J' | b'P' | b'@' => return true,
    ...
    fn is_safe_ss3_sequence(byte: u8) -> bool {
...
mod interface_tests {
    ...
    impl MockExecInterface {
        fn new() -> Self {
            Self {
                ...
                dimensions: Some(TerminalDimensions { width: 80, height: 24 }),
            }
        }
    ...
    impl MockTerminalHandler {
        fn new(tty_available: bool) -> Self {
            Self {
                ...
                terminal_input: Arc::new(Mutex::new(vec![
    ...
    fn test_terminal_dimensions() {
        let dims = TerminalDimensions { width: 120, height: 40 };
    ...
    fn test_exec_mode() {
        ...
        // Test External mode
        let external = ExecMode::External(id.clone());
        match &external {
