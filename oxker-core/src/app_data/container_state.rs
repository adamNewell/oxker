...
/// A simple Size struct to replace ratatui's Size
...
impl Logs {
    ...
    pub fn insert(&mut self, line: String, tz: LogsTz) {
        if self.tz.insert(tz) {
            self.max_log_len = self.max_log_len.max(line.len());
    ...
    /// If offset set, remove `char_offset` number of chars from a string
    fn format_log_line(text: &str, char_offset: usize, width: u16) -> &str {
        let width = width as usize;
        if char_offset >= text.len() {
            ""
        } else {
            let end = (char_offset + width).min(text.len());
            &text[char_offset..end]
        }
    }
        ...
        self.lines
            ...
            .map(|(index, item)| {
                if current_index.abs_diff(index) <= height_padding {
                    Self::format_log_line(item, char_offset, size.width).to_string()
                } else {
                    String::new()
                }
            })
...
mod tests {
    ...
    fn test_container_state_logz() {
        ...
        logs.insert(line.clone(), tz.clone());
        logs.insert(line.clone(), tz.clone());
        logs.insert(line, tz);
